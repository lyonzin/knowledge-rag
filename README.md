...
### v4.5.2 (2026-07-30) — FastMCP package migration (unblock 2026-07-28 follow-up)

- **FIX (deps)**: replace the removed `mcp.server.fastmcp` import path with `fastmcp-slim[mcp]>=3.4.5,<4.0.0` — the dependency-slim variant of the standalone FastMCP package (5 transitive deps instead of ~20 pulled by `fastmcp` full via `[client,server]` extras). Upstream `mcp 2.0.0` (published 2026-07-28) relocated `mcp.server.fastmcp`; the pin `mcp<2.0.0` added in v4.5.0-post kept CI green but blocked any future consumer from installing knowledge-rag alongside a modern `mcp` 2.x. Delivers the follow-up promised in v4.5.0-post without adopting a beta SDK, and without the ~20% RSS overhead the full `fastmcp` package would have introduced.
- **CHANGE (imports)**: `mcp_server/server.py` now imports `FastMCP` from `fastmcp` instead of `mcp.server.fastmcp`. All 13 MCP tool signatures, all tool names, and every public function under `mcp_server.*` are **frozen and unchanged** — `check_api_surface.py --check` passes clean, `test_backwards_compat.py` stays green.
- **CHANGE (constructor)**: `FastMCP(...)` no longer accepts `host` / `port` in 3.x (breaking change from `fastmcp` 3.x itself, not from this repo). `host`/`port` are now passed to `mcp.run(transport=..., host=..., port=...)` on the non-stdio branches only. stdio users see zero behaviour change.
- **NEW (metadata)**: `FastMCP(name="knowledge-rag", version="4.5.2")` — server now advertises its version explicitly, matching the direction the MCP spec 2026-07-28 (`serverInfo` in responses) is heading. Full stateless-core adoption of the 2026-07-28 spec (per-request `_meta.io.modelcontextprotocol/protocolVersion`, retirement of `initialize`/`initialized` + `Mcp-Session-Id`) waits for `fastmcp` 4.x to leave beta — tracked as a follow-up.
- **PERF**: chose `fastmcp-slim[mcp]` over `fastmcp` full to avoid ~15 extra transitive dependencies (`fastmcp` full pulls `fastmcp-slim[client,server]`). Keeps the pre-migration RSS baseline; `test_bench_orchestrator_idle_rss` and `test_bench_query_cache_5000_entries` stay within the 10% regression gate.
- **DOCS**: `config.yaml` example bundles code-format extensions (`.c` / `.h` / `.cpp` / `.js` / `.jsx` / `.ts` / `.tsx` / `.xml`) that were already in `_SUPPORTED_SUFFIXES` and in `Config.supported_formats` defaults but not surfaced in the shipped example — brings the user-facing config in line with the internal capability set already covered by `test_new_code_formats_default_enabled`.
- **VERSION**: 4.5.1 → **4.5.2** (PATCH — foundation-only refactor, unblocks the 2026-07-28 migration path; every public API + every MCP tool signature preserved).
