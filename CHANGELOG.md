...
- **fix(config)** — the `init`-generated `config.yaml` now enables the full default format set (all 33 non-MetaTrader formats) instead of only `.md`/`.txt`/`.pdf`/`.docx`. The bundled `mcp_server/data/config.example.yaml` template previously shipped an explicit list with the new infra/code formats commented out, so anyone who ran `init` silently opted out of Go/Rust/YAML/Proto/SQL/etc; it now mirrors the root `config.example.yaml`. Also corrects the stale "20 formats" count to 35 across `README.md` and `docs/ARCHITECTURE.md`, and `12 MCP tools` to 13 in the npm README.
