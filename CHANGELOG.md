...
- **fix(watcher)** — GH #214: the file watcher no longer enqueues a full incremental reindex for metadata-only events. Watchdog's `on_modified` fires for every `IN_ATTRIB`-class change (chmod, xattr, `utimensat` that keeps mtime), and external processes generate a constant stream of them — cloud-sync clients (OneDrive, Dropbox, Nextcloud), backup agents, antivirus, and OS file indexers (`tracker3`, Windows Search) all touch attributes on read. Before this fix each such event triggered `index_all(force=False)` which scanned every indexed file only to skip them all, producing an infinite `[WATCHER] N file(s) changed → [INDEX] Progress: N/N (100%) — 0 new, N skipped` loop that could saturate CPU for hours (5019-file corpus in the report). `DocumentWatcher.on_modified` now consults the orchestrator's `(mtime, size)` state for known paths and drops events whose on-disk stat still matches the last-indexed value; `on_created`, `on_deleted`, and `on_moved` remain unconditional because those events always signal a real change. 12 regression tests cover the drop path, mtime/size drift, unknown paths, stat failures, orchestrator-not-ready, and the unchanged behaviour of the other three handlers.
